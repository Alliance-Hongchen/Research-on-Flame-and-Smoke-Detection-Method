...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in {-1, 0}:
        ...
        test_loader = create_dataloader(test_path,  # 使用测试集路径
                                        imgsz,
                                        batch_size // WORLD_SIZE,  # 或者保持与训练批次大小相同，根据需求调整
                                        gs,
                                        single_cls,
                                        hyp=hyp,
                                        cache=None if noval else opt.cache,  # 根据是否缓存测试集数据调整
                                        rect=True,  # 保持与验证时相同的设置
                                        rank=-1,
                                        workers=workers * 2,  # 可能需要根据硬件资源调整工作线程数
                                        pad=0.5,
                                        prefix=colorstr('test: '),  # 更改日志前缀以反映正在加载的是测试数据
                                        shuffle=False,  # 测试集一般不需打乱
                                        # 其他特定于测试集的参数可以根据实际情况添加
                                        )[0]
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default='/root/yolov9-main/yolov9-c-converted.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='/root/yolov9-main/models/detect/yolov9-MobilenetV4.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='/root/yolov9-main/data/coco.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default='/root/yolov9-main/data/hyps/hyp.scratch-high.yaml', help='hyperparameters path')
